...
from utils import get_model, MNISTM_Model
...
parser.add_argument('--data_dir', default='/nobackup/yguo/datasets', type=str,
...
parser.add_argument('--pgd_num_steps', default=7, type=int,
                    help='number of pgd steps in adversarial training')
parser.add_argument('--pgd_step_size', default=0.031/4,
...
parser.add_argument('--aux_data_filename', default='mnistm', type=str,
...
image_root = '/nobackup/yguo/datasets/mnist_m'
test_set = GetLoader(
...
eval_test_loader = DataLoader(test_set, batch_size=args.test_batch_size,
                              shuffle=False, **kwargs)
# ------------------------------------------------------------------------------
            ...
            if batch_idx < args.eval_attack_batches:
                if args.distance == 'l_2':
                    ...
                    is_correct_clean, is_correct_rob = pgd(
                        model, data, target,
                        epsilon=args.epsilon,
                        num_steps=2 * args.pgd_num_steps + 6,
                        step_size=args.pgd_step_size,
                        random_start=False)
