...
class SemiSupervisedDataset(Dataset):
    def __init__(self,
        ...
        if self.train:
            ...
            if aux_data_filename == 'mnistm':  
                self.data = np.repeat(
                    self.data[:, np.newaxis, :, :], 3, axis=1)
                train_list = os.path.join(
                    image_root, 'mnist_m_test_labels.txt')
                ...
                train_dataloader = torch.utils.data.DataLoader(
                    ...
                    shuffle=True,
                    num_workers=8) 
                aux_data, _ = next(iter(train_dataloader))   
                add_aux_labels = None             
            ...
            #         aux_targets = aux_targets[take_inds]
                ...
                self.targets = list(self.targets.cpu().numpy())
